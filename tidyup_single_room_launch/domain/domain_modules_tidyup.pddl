(define (domain tidyup)
    ...
    (:predicates
        ...
        (grasp-impossible ?o - movable_object ?l - manipulation_location ?a - arm)  ; is ?o graspable from ?g with ?a
        (reachable ?o - movable_object ?l - manipulation_location)  ; is the object in range of the robots arms form this location
        ...
        (object-inspectable ?o - movable_object)     ; object was brought to the camera for inspection
    ...
    (:durative-action pickup-object
        ...
        (and
            ...
            (at start (reachable ?o ?l))
            (at start (not (grasp-impossible ?o ?l ?a)))
    ...
    (:durative-action putdown-object
        ...
        (and
            ...
            (at end (not (searched ?l)))
            (at end (not (recent-detected-objects ?l)))
            ;(at end (reachable ?o ?l))
        )
    )

    (:durative-action arm-to-inspect-object
...
;                    (not (grasp-impossible ?o ?l ?a)))                          ; we can somehow grasp the object
;                )
;            )
;        )
;    )
