...
static const unsigned int DIFFS_SIZE = 3;
static const bool ATTACH_DEBUG = false;
...
void Gripper::Load(sdf::ElementPtr _sdf)
{
  ...
  while (gripperLinkElem)
  {
    ...
    for (unsigned int j = 0; j < gripperLink->GetChildCount(); ++j)
    {
      ...
      gripperLinks.push_back(collision->GetScopedName());
    }
  ...
  if (!this->collisions.empty())
  {
    ...
    std::string topic = mgr->CreateFilter(this->GetName(), gripperLinks);
    if (!this->contactSub)
    {
...
void Gripper::OnUpdate()
{
  ...
  if(ATTACH_DEBUG) {
      gzdbg << "OnUpdate " << this->name << " attached: " << this->attached <<
          " contacts: " << this->contacts.size() <<
          " cur pos: " << this->posCount << " cur neg: " << this->zeroCount << " \n";
  }
  if (this->posCount > this->attachSteps && !this->attached) {
    if(ATTACH_DEBUG)
        gzdbg << "Trying to attach\n";
    this->HandleAttach();
  }
  else if (this->zeroCount > this->detachSteps && this->attached) {
    if(ATTACH_DEBUG)
        gzdbg << "Trying to detach\n";
    this->HandleDetach();
  }
  if(ATTACH_DEBUG)
      gzdbg << "Object is now attached: " << this->attached << "\n";
  ...
  while (iter != contactCounts.end())
  {
    ...
    else
    {
      if (!this->attached && cc[iter->first])
      {
        ...
        this->diffIndex = (this->diffIndex+1) % DIFFS_SIZE;
      }
      ++iter;
    }
  }
